...
const Auth = (() => {
  ...
  function register(name, email, password) {
    ...
    if (users[email]) return { ok: false, msg: T('auth.err_email_taken', 'Этот email уже зарегистрирован') };
  ...
  function login(email, password) {
    const users = getUsers();
    const u = users[email];
    if (!u) return { ok: false, msg: T('auth.err_user_not_found', 'Пользователь не найден') };
    if (u.password !== password) return { ok: false, msg: T('auth.err_wrong_password', 'Неверный пароль') };
  ...
  function injectModal() {
    ...
    <div id="authOverlay" class="auth-overlay" onclick="Auth.closeAuthModal(event)">
      <div class="auth-modal" id="authModal">
        <button class="auth-modal-close" onclick="Auth.closeAuthModal(true)" aria-label="${T('auth.close', 'Закрыть')}">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        </button>

        <!-- Tabs -->
        <div class="auth-tabs">
          <button class="auth-tab active" id="tabLogin" onclick="Auth.switchTab('login')">${T('auth.login', 'Войти')}</button>
          <button class="auth-tab" id="tabReg" onclick="Auth.switchTab('reg')">${T('auth.register', 'Регистрация')}</button>
        </div>

        <!-- Login form -->
        <div id="formLogin" class="auth-form-section">
          <p class="auth-subtitle">${T('auth.login_welcome', 'Добро пожаловать!')}</p>
          ...
          <button class="auth-submit btn btn-primary" onclick="Auth.submitLogin()">${T('auth.login', 'Войти')}</button>
        </div>

        <!-- Register form -->
        <div id="formReg" class="auth-form-section" style="display:none">
          <p class="auth-subtitle">${T('auth.register_title', 'Создайте аккаунт')}</p>
          <div class="auth-field">
            <label>${T('auth.name', 'Имя')}</label>
            <input type="text" id="regName" placeholder="${T('auth.name_placeholder', 'Ваше имя')}" autocomplete="name">
          </div>
          <div class="auth-field">
            <label>${T('auth.email', 'Email')}</label>
          ...
          <div class="auth-field">
            <label>${T('auth.password', 'Пароль')}</label>
            <input type="password" id="regPass" placeholder="${T('auth.password_placeholder_min', 'Минимум 6 символов')}" autocomplete="new-password">
          </div>
          <div class="auth-error" id="regError"></div>
          <button class="auth-submit btn btn-primary" onclick="Auth.submitReg()">${T('auth.create_account', 'Создать аккаунт')}</button>
        </div>
      </div>
    </div>
    ...
    <div id="userDropdown" class="user-dropdown" style="display:none">
      ...
      <a class="user-dropdown-item" href="favorites.html" onclick="Auth.closeDropdown()">
  ...
  function renderAuthBtn() {
    ...
    if (user) {
      ...
      container.innerHTML = `
        <button class="user-avatar-btn" id="userAvatarBtn" onclick="Auth.toggleDropdown()" title="${user.name}" aria-label="${T('auth.account', 'Личный кабинет')}">
          ${initials}
        </button>
      `;
    } else {
      container.innerHTML = `
        <button class="auth-icon-btn" id="authIconBtn" onclick="Auth.openAuthModal()" title="${T('auth.login', 'Войти')}" aria-label="${T('auth.login', 'Войти')}">
  ...
  function submitLogin() {
    ...
    if (!email || !pass) { errEl.textContent = T('auth.err_fill_all', 'Заполните все поля'); return; }
    const res = login(email, pass);
    if (!res.ok) { errEl.textContent = res.msg; return; }
    closeAuthModal(true);
    renderAuthBtn();
    showAuthToast(T('auth.toast_welcome', `Добро пожаловать, ${res.session.name}!`, { name: res.session.name }));
  }
  ...
  function submitReg() {
    ...
    if (!name || !email || !pass) { errEl.textContent = T('auth.err_fill_all', 'Заполните все поля'); return; }
    if (pass.length < 6) { errEl.textContent = T('auth.err_password_min', 'Пароль — минимум 6 символов'); return; }
    if (!/\S+@\S+\.\S+/.test(email)) { errEl.textContent = T('auth.err_invalid_email', 'Введите корректный email'); return; }
    const res = register(name, email, pass);
    if (!res.ok) { errEl.textContent = res.msg; return; }
    closeAuthModal(true);
    renderAuthBtn();
    showAuthToast(T('auth.toast_account_created', `Аккаунт создан! Добро пожаловать, ${name}!`, { name }));
  }
